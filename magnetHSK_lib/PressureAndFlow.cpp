...
void initRS232() {
	Serial1.begin(9600); // Flow 1 (Stack)
...
bool pressureRawRequest(char *request, char responseString[200], char error[100]) {
	int rindex = 0;
	int timeout = 0;
	char chunk;
...
bool readPressureSensor(double *value, double *tempVal, char *pressureType, char error[100]){
...
bool flowRawRequest(int flowNum, char *request, char responseString[200], char error[100]) {
	int rindex = 0;
	int timeout = 0;
	char chunk;
