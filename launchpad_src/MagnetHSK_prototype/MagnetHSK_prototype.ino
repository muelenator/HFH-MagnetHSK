 ...
 * reporting, with additional responses & functions that are magnetHSK specific.
 *
 *
 */
...
void commandCenter()
{
	...
	/* Check magnet housekeeping commands */
	else if ((int) eResistanceCh3 <= (int) hdr_in->cmd <= (int) eResistanceCh20)
	{
		whatToDoIfResistance(hdr_out, hdr_in->cmd);
	}
	else if ((int) eTempCh3 <= (int) hdr_in->cmd <= (int) eTempCh20)
	{
		whatToDoIfTemp(hdr_out, hdr_in->cmd);
	}
	else if ((int) eTempProbe1 <= (int) hdr_in->cmd == (int) eTempProbe10)
	{
		whatToDoIfTempProbe(hdr_out, hdr_in->cmd);
	}
	else if ((int) eFlow1 <= (int) hdr_in->cmd <= (int) eFlow2)
	{
		if (whatToDoIfFlow(hdr_out, hdr_in->cmd))
		{
			fillChecksum((uint8_t *) outgoingPacket);
			downStream1.send(outgoingPacket, hdr_size + hdr_out->len + 1);
		}
	}
	else if (hdr_in->cmd == ePressure)
	{
		whatToDoIfPressure(hdr_out);
	}
